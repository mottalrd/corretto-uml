...
public class SOrder implements BooleanFormulae{
...
//    	return s;
    ...
    public BooleanFormulae getFun() {
    	if (guard == null) {
    		if (isConcurrent)
    			return new OrderCon(f1, f2, exception);
    		else
    			return new OrdernoCon(f1, f2, exception);
    	}else {
    		if (isConcurrent)
    			return new OrderGCon(f1, f2, guard, exception);
    		else
    			return new OrderGnoCon(f1, f2, guard, exception);
    	}
    }
